...
def cleanup_resources(result_manager=None, processor=None, patent_processor=None):
    """清理内存资源和GPU缓存"""
    # 清理结果管理器中的数据
    ...
    if patent_processor is not None:
        patent_processor.processor = None
...
def process_single_patent():
    """处理单个专利目录"""
    patent_dir = "/home/zhangxiaohong/zhouxingyu/zxy_extractor/data/temp_output/CN103304492"  
    # 创建结果管理器
    ...
    # 处理完成后清理资源
    cleanup_resources(result_manager, processor, patent_processor)


def process_multiple_patents():
    """处理多个专利目录"""
    input_root = "/home/zhangxiaohong/zhouxingyu/zxy_extractor/data/temp_output/drug_CN_100"  
    output_root = "/home/zhangxiaohong/zhouxingyu/zxy_extractor/data/temp_output/drug_CN_100"  
    ...
    # 释放FolderProcessor资源
    if hasattr(folder_processor, 'processors'):
        for key in list(folder_processor.processors.keys()):
            del folder_processor.processors[key]
        folder_processor.processors.clear()
...
def process_single_image():
    ...
    # 处理完成后清理资源
    cleanup_resources(result_manager, processor)
